/**
 * Tests for Secrets Change Extractor
 *
...
describe('secretsChangeExtractor', () => {
	describe('shouldRun', () => {
		it('returns true when relationships tracking is enabled and at correct message interval', () => {
			...
			expect(secretsChangeExtractor.shouldRun(context)).toBe(true);
		});
		...
		it('returns false when not at the correct message interval', () => {
			...
			expect(secretsChangeExtractor.shouldRun(context)).toBe(false);
		});
	...
	describe('extractor properties', () => {
		it('has correct name and category', () => {
			expect(secretsChangeExtractor.name).toBe('secretsChange');
			expect(secretsChangeExtractor.category).toBe('relationships');
		});

		it('has moderate temperature', () => {
			expect(secretsChangeExtractor.defaultTemperature).toBe(0.6);
		});

		it('uses fixedNumber message strategy with 4 messages', () => {
			expect(secretsChangeExtractor.messageStrategy).toEqual({
		...
		it('uses everyNMessages run strategy with 4 messages', () => {
			expect(secretsChangeExtractor.runStrategy).toEqual({
...
describe('secretsChangeExtractor message limiting', () => {
	...
	it('limits messages to maxMessagesToSend when context has more messages', async () => {
		...
		await secretsChangeExtractor.run(
			mockGenerator,
			context,
			settings,
			store,
