/**
 * Tests for Position Change Extractor
 *
 * Focuses on shouldRun logic and extractor properties.
 */

import { describe, it, expect, vi } from 'vitest';
import { positionChangeExtractor } from './positionChangeExtractor';
...
describe('positionChangeExtractor', () => {
	describe('shouldRun', () => {
		it('returns true when characters tracking is enabled', () => {
			...
			expect(positionChangeExtractor.shouldRun(context)).toBe(true);
		});
		...
		it('returns false when characters tracking is disabled', () => {
			...
			expect(positionChangeExtractor.shouldRun(context)).toBe(false);
		});
	});

	describe('extractor properties', () => {
		it('has correct name and category', () => {
			expect(positionChangeExtractor.name).toBe('positionChange');
			expect(positionChangeExtractor.category).toBe('characters');
		});

		it('has moderate temperature', () => {
			expect(positionChangeExtractor.defaultTemperature).toBe(0.5);
		});

		it('uses fixedNumber message strategy with 1 message', () => {
			expect(positionChangeExtractor.messageStrategy).toEqual({
		...
		it('uses everyMessage run strategy', () => {
			expect(positionChangeExtractor.runStrategy).toEqual({
