/**
 * Location Change Event Extractor Tests
 *
 * Tests that verify the location change extractor detects location changes
 * and emits LocationMovedEvent.
 */
...
import type { MessageAndSwipe, Snapshot, Event, LocationMovedEvent } from '../../types';
...
function createMockContext(overrides: Partial<ExtractionContext> = {}): ExtractionContext {
	return {
		chat: [
			...
			{
				mes: '*Elena walks outside to the patio, taking a seat at one of the tables.*',
				is_user: false,
				is_system: false,
				name: 'Elena',
			},
...
describe('locationChangeExtractor', () => {
	...
	describe('shouldRun', () => {
		it('returns true when location tracking is enabled', () => {
			...
			expect(locationChangeExtractor.shouldRun(runContext)).toBe(true);
		});
		...
		it('returns false when location tracking is disabled', () => {
			...
			expect(locationChangeExtractor.shouldRun(runContext)).toBe(false);
		});
	});

	describe('run', () => {
		it('returns LocationMovedEvent when location change is detected', async () => {
			const context = createMockContext();
			const settings = createMockSettings();
			...
			mockGenerator.setDefaultResponse(
				JSON.stringify({
					reasoning: 'Elena moved from the coffee shop inside to the patio outside.',
					changed: true,
					newArea: 'Downtown',
					newPlace: 'Patio',
					newPosition: 'seated at a table',
				}),
			);

			const result = await locationChangeExtractor.run(
				mockGenerator,
				context,
				settings,
				store,
			...
			expect((result[0] as LocationMovedEvent).subkind).toBe('moved');
		});

		it('returns empty array when no location change detected', async () => {
			const context = createMockContext();
			const settings = createMockSettings();
			const currentMessage: MessageAndSwipe = { messageId: 2, swipeId: 0 };
			...
			mockGenerator.setDefaultResponse(
				JSON.stringify({
					reasoning: 'No location change detected.',
					changed: false,
				}),
			);
		...
		it('passes current location to the prompt', async () => {
			const context = createMockContext();
			const settings = createMockSettings();
			const currentMessage: MessageAndSwipe = { messageId: 2, swipeId: 0 };
	...
	describe('extractor configuration', () => {
		it('has the correct name', () => {
			expect(locationChangeExtractor.name).toBe('locationChange');
		});

		it('has the correct category', () => {
			expect(locationChangeExtractor.category).toBe('location');
		});

		it('has a default temperature', () => {
			expect(locationChangeExtractor.defaultTemperature).toBe(0.5);
		});

		it('uses fixedNumber message strategy with n=2', () => {
			expect(locationChangeExtractor.messageStrategy).toEqual({
		...
		it('uses everyMessage run strategy', () => {
			expect(locationChangeExtractor.runStrategy).toEqual({
